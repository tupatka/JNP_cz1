...
void create_graph(vector<pair<vector<int32_t>, vector<int32_t>>> &graph, unordered_map<int32_t, signal> &signals,
    ...
    for (size_t i = 0; i < num_of_gates; i++) {
        ...
        for (auto neighbouring_gate: gates[i].first) {
            if (signals.count(neighbouring_gate) == 0) {
                signals[neighbouring_gate].first = -1;
            }
            signals[neighbouring_gate].second.push_back(i);
        }
    }
...
bool visit(int32_t a, vector<visited_state> &visited_states, vector<pair<vector<int32_t>, vector<int32_t> > > &graph) {
    visited_states[a] = visited_state::IN_PROGRESS;
    for (auto neighbour: graph[a].second) {
        if (visited_states[neighbour] == visited_state::IN_PROGRESS) {
            // Znalezienie cyklu.
            return false;
        } else if (visited_states[neighbour] == visited_state::NOT_BEGAN) {
            if (!visit(neighbour, visited_states, graph)) {
                // Znalezienie cyklu w rekurencyjnych wywołaniach.
                return false;
            }
        }
    }
    visited_states[a] = visited_state::FULLY_DONE;
    return true;
}

// Funkcja pomocnicza, ustawia wszystkie wierzchołki jako nieodwiedzone.
void clear_table(vector<visited_state> &visited_states, size_t num_of_gates) {
    for (size_t i = 0; i < num_of_gates; i++) {
        visited_states.push_back(visited_state::NOT_BEGAN);
    }
}
...
bool is_cyclic(size_t num_of_gates, vector<pair<vector<int32_t>, vector<int32_t>>> &graph) {
    ...
    vector<visited_state> visited_states;
...
void initialize_state_of_signals(vector<bool> &state_of_signals, size_t num_of_input_signals) {
    for (size_t i = 0; i < num_of_input_signals; i++) {
        state_of_signals.push_back(false);
    }
}

// Zmienia tablicę z konfiguracją sygnałów na nastepną
// Zwraca false jeśli była to już ostatnia.
bool next_state_of_signals(vector<bool> &state_of_signals, size_t num_of_input_signals) {
...
void toposort(vector<int32_t> &sorted_vertices, size_t num_of_gates, vector<pair<vector<int32_t>,
    ...
    while (!zero_degree_vertices.empty()) {
        ...
        for (auto in_vertex: graph[v].first) {
            vertex_degrees[in_vertex]--;
            if (vertex_degrees[in_vertex] == 0) {
                zero_degree_vertices.push(in_vertex);
            }
        }
    }
...
void compute_output(vector<int32_t> &sorted_gates, size_t num_of_gates, vector<gate> &gates,
    ...
    for (auto gate: sorted_gates) {
        complete(gate, gates, gate_types, truth_output);
    }
}
...
void complete_input_values(size_t num_of_input_signals, vector<int32_t> &input_signals,
                           vector<bool> &state_of_signals, map<int32_t, bool> &truth_output) {
    for (auto i = truth_output.begin(); i != truth_output.end(); ++i) {
        i->second = false;
    }
    for (size_t i = 0; i < num_of_input_signals; i++) {
